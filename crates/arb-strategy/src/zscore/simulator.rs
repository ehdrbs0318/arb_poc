...
impl<U: MarketData, B: MarketData> BacktestSimulator<U, B> {
    ...
    pub async fn run(&self) -> Result<BacktestResult, StrategyError> {
        ...
        let total_candles = self.config.total_candles_needed();
        let end_time = Utc::now();

        info!(
            coins = ?self.config.coins,
            window_size = self.config.window_size,
            backtest_minutes = self.config.backtest_period_minutes,
            total_candles = total_candles,
            "백테스트 시작"
        );

        // 1. 데이터 수집 (코인별 + USDT/KRW)
        ...
        info!(
            usdt_krw_count = usdt_krw_candles.len(),
            "USDT/KRW 캔들 데이터 수집 완료"
        );

        // 2. 시간 정렬: 모든 시계열의 공통 타임스탬프 생성
        let timestamps =
            self.build_aligned_timestamps(&upbit_coin_candles, &usdt_krw_candles, &bybit_candles);

        if timestamps.len() < self.config.window_size {
        ...
        info!(
            ...
            warmup = self.config.window_size,
            simulation = timestamps.len().saturating_sub(self.config.window_size),
            "시간 정렬 완료"
        );

        // 3. SpreadCalculator + PositionManager 초기화
        ...
        for (idx, &ts) in timestamps.iter().enumerate() {
            let is_warmup = idx < self.config.window_size;

            for coin in &self.config.coins {
                // 각 소스에서 해당 timestamp의 close 조회
                let upbit_close = upbit_coin_candles
                    .get(coin)
                    .and_then(|candles| find_close_at(candles, ts));
                let usdt_krw_close = find_close_at(&usdt_krw_candles, ts);
                ...
                if let Some(pos) = position_mgr.open_positions.get(coin) {
                    ...
                    if holding > (self.config.window_size * 2) as u64 {
                        warn!(
                            coin = coin.as_str(),
        ...
        for pos in &open_positions {
            ...
            let fees = pos.size_usdt
                * (self.config.upbit_taker_fee + self.config.bybit_taker_fee)
                * Decimal::from(2u64);
        ...
        let warmup_end = if timestamps.len() > self.config.window_size {
            timestamps[self.config.window_size]
        } else {
            *timestamps.last().unwrap_or(&end_time)
        };
        let test_period_end = *timestamps.last().unwrap_or(&end_time);
